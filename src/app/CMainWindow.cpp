    ...
    , m_useShade(false)
{
    ...
    QTimer::singleShot(300, [this]() { 
        ...
        {
            CMessageBox::info("录制视频目录所在磁盘占用空间超过了80%\n不允许录制视频！");
        }
    });
...
void CMainWindow::initQAction()
{
    ...
    connect(m_recordDirAct, &QAction::triggered, []() {
        ...
        if (!dir.exists())
        {
            if (!dir.mkpath(path))
            {
                QString msg = QString("创建目录%1失败").arg(path);
                qInfo() << msg;
...
void CMainWindow::refreshBtnCmd()
{
    ...
    case 0:
        ui->btn_cmd->setText("开始录制");
        break;
    case 1:
        ui->btn_cmd->setText("结束录制");
        break;
    }
}
...
void CMainWindow::onBtnClose()
{
    if (m_concatProcessNum > 0) {
        CMessageBox::info("正在生成录制视频！\n请稍候再关闭");
        return;
    }

    if (CMessageBox::confirm("是否确认退出？"))
    {
        close();
    }
}
...
void CMainWindow::onBtnCmd()
{
    ...
    QString text;
    switch (m_recordStarted) {
    case 0: text = "是否确认开始录制？"; break;
    case 1:  text = "是否确认结束录制？"; break;
    }
...
{
    if (CMessageBox::confirm("是否打开所有摄像头？"))
    {
        qInfo() << "手动打开所有摄像头";
        ui->page_1->openAll();
    }
...
{
    if (CMessageBox::confirm("是否关闭所有摄像头？"))
    {
        qInfo() << "手动关闭所有摄像头";
        ui->page_1->closeAll();
    }
