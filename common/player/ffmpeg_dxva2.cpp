...
typedef struct DXVA2SurfaceWrapper {
    ...
    IDirectXVideoDecoder* decoder;
    IPlayerCore*          iPlayerCore = nullptr;
} DXVA2SurfaceWrapper;
...
static void dxva2_release_buffer(void *opaque, uint8_t *data)
{
    ...
    //QMutexLocker lk(&w->iPlayerCore->m_dxvaCbMtx);
    ...
    {
        //qDebug() << "w->iPlayerCore->m_dxvaCbEnabled false";
        //return;
    }
...
static int dxva2_get_buffer(AVCodecContext *s, AVFrame *frame, int flags)
{
    ...
    if (!iPlayerCore->m_dxvaCbEnabled)
    {
        return -1;
    }
...
static int dxva2_retrieve_data(AVCodecContext *s, AVFrame *frame)
{
    ...
    IPlayerCore* iPlayerCore = (IPlayerCore*)s->opaque;
    if (nullptr == iPlayerCore || nullptr == iPlayerCore->m_inputStream)
    {
        return -1;
    }

    IDirect3DSurface9 *pBackBuffer = iPlayerCore->m_pBackBuffer;
    DXVA2Context       *ctx = (DXVA2Context *)iPlayerCore->m_inputStream->hwaccel_ctx;
	...
	GetClientRect(iPlayerCore->m_d3dpp.hDeviceWindow, &m_rtViewport);
...
static int dxva2_alloc(AVCodecContext *s, HWND hwnd)
{
    ...
    //D3DPRESENT_PARAMETERS d3dpp = iPlayerCore->m_d3dpp;
    ...
    iPlayerCore->m_d3dpp.Windowed = TRUE;
    iPlayerCore->m_d3dpp.BackBufferCount = 0;
    iPlayerCore->m_d3dpp.hDeviceWindow = hwnd;
    iPlayerCore->m_d3dpp.SwapEffect = D3DSWAPEFFECT_DISCARD;
    iPlayerCore->m_d3dpp.BackBufferFormat = d3ddm.Format;
    iPlayerCore->m_d3dpp.EnableAutoDepthStencil = FALSE;
    iPlayerCore->m_d3dpp.Flags = D3DPRESENTFLAG_VIDEO;
    iPlayerCore->m_d3dpp.FullScreen_RefreshRateInHz = D3DPRESENT_RATE_DEFAULT;
    iPlayerCore->m_d3dpp.PresentationInterval = D3DPRESENT_INTERVAL_IMMEDIATE;
	...
	hr = IDirect3D9_CreateDevice(ctx->d3d9, adapter, D3DDEVTYPE_HAL, hwnd, BehaviorFlags, &iPlayerCore->m_d3dpp, &ctx->d3d9device);
    if (FAILED(hr)) {
...
#if 1
    if (!ctx) {
        qDebug() << "DXVA2Context nullptr!!!";
        return;
    }
#endif

    // FIXME: 为什么会出现ctx为nullptr,然后崩溃
    if (ctx->decoder)
        dxva2_destroy_decoder(s);
